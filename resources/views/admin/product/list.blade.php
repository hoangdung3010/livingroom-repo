                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-3 mb-0" style="min-width:100px;">
                                                <select id="" name="fill_action" class="form-control">
                                                    ...
                                                    <option value="ban_chay" {{ $fill_action=='ban_chay'? 'selected':'' }}>Sản phẩm bán chạy</option>
                                                    <option value="no_ban_chay" {{ $fill_action=='no_ban_chay'? 'selected':'' }}>Sản phẩm không tích bán chạy</option>
                                                </select>
                                            </div>
                                            <div class="form-group col-md-3 mb-0" style="min-width:100px;">
                        ...
                        <table class="table table-head-fixed" style="font-size: 13px;">
                            <thead>
                                <tr>
                                    ...
                                    {{-- <th class="white-space-nowrap">User tạo</th> --}}
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><img src="{{asset($productItem->product_img)}}" alt="{{$productItem->name}}" style="width:80px;"></td>
                                    <td class="wrap-load-active" data-url="{{ route('admin.product.load.active',['id'=>$productItem->id]) }}">
                                     ...
                                     {{-- <td class="wrap-load-banchay" data-url="{{ route('admin.product.load.banchay',['id'=>$productItem->id]) }}">
                                        @include('admin.components.load-change-ban-chay',['data'=>$productItem,'type'=>'sản phẩm'])
                                     </td> --}}
                                     {{-- <td>
                                         @if ($productItem->user_id)
                                         <a href="{{ route('admin.user_frontend.detail',['id'=>$productItem->user->id]) }}">{{ optional($productItem->user)->username }}</a>
                                         @endif
                                     </td> --}}

                                    <td>{{optional($productItem->category)->name}}</td>
                                     {{-- <td>
                                        @if (optional($productItem->user)->type==4)
                                          {{ $productItem->getTotalClickBuy() }}
                                        @endif
                                     </td> --}}
